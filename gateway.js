...
const servers_deploy = require('./servers_deploy.json');
const lang_detection = require('./src/javascript/lang_detection');
const sentence_extractor = require('./src/javascript/sentence_extractor');
...
app.post("/ner/tokenize", (req, res) => {
  ...
  const server_address = servers_deploy[params.lang];
  axios.post('http://' + server_address + '/ner/tokenize', params)
...
app.post("/ner/extract_entities", (req, res) => {
  ...
  const server_address = servers_deploy[params.lang];
  axios.post('http://' + server_address + '/ner/extract_entities', params)
  .then(function (response) {
    let temp = {
      "status": "OK",
      "data": response.data
...
swaggerDocument.info.title = "Jace NER Service";
swaggerDocument.info.description = "Provides API for Named Entity Recognition";
swaggerDocument.host = config.service.host + ':' + config.service.port
